...
class Common:
    ...
    def is_valid_date_reserve(date_utc):
        return date_utc > Common.get_limit_date_reserve()

    def get_limit_date_reserve():
    ...
    def get_paginator(request, model, per_page=settings.PAGINATOR_PER_PAGE): #settings.PAGINATOR_PER_PAGE
